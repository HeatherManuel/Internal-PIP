...
import {
  ...
  Clock, ChevronDown, ChevronUp, FolderOpen,
} from 'lucide-react'
...
type Tab = 'files' | 'sync' | 'generate'

// ── Sync Panel ────────────────────────────────────────────────────────────────

function DriveSyncPanel({ onSwitchToFiles }: { onSwitchToFiles: () => void }) {
  const {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-gray-800/40 rounded-lg p-3 text-center">
          ...
          <p className="text-xs text-gray-500">Chunks in knowledge base</p>
        </div>
      </div>
      ...
      {stats.readyToSync === 0 && (
        <div className="flex items-start gap-2 bg-yellow-950/30 border border-yellow-900/50 rounded-lg px-3 py-2.5 text-xs text-yellow-400">
          ...
          <span>
            ...
            <button onClick={onSwitchToFiles} className="underline hover:no-underline">
              Go to File Manager
            </button>{' '}
...
function GeneratePanel() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // TODO: call Edge Function → embed brief → match_voice_chunks → Claude
      ...
      <div className="pip-card space-y-4">
        {totalChunks === 0 && (
          <div className="flex items-center gap-2 bg-yellow-950/30 border border-yellow-900/50 rounded-lg px-3 py-2 text-xs text-yellow-400">
            ...
            Knowledge base is empty — sync your Drive files first.
          </div>
        )}
        <div>
        ...
        {!output && !loading && (
          <div className="flex flex-col items-center justify-center h-64 text-center">
            ...
            <p className="text-gray-500 text-sm max-w-48">
              Classify your Drive files, sync them, then generate content that sounds like <em>you</em>
            </p>
          </div>
        )}
        {loading && (
...
export function VoiceAgent() {
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode; badge?: string }[] = [
    {
      ...
      badge: stats.unclassified > 0 ? String(stats.unclassified) : undefined,
    },
    {
      id:    'sync',
      label: 'Sync',
  ...
  return (
    <div>
      <Header
        title="🎙️ Voice Agent"
        subtitle="Classify your Drive content, sync it, then generate copy in your voice."
      />
      ...
      {activeTab === 'sync' && (
        <div className="pip-card max-w-lg">
          <DriveSyncPanel onSwitchToFiles={() => setActiveTab('files')} />
        </div>
      )}
